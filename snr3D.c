...
void compute_measures (	char **argv,
	...
	if(FFT) {
		...
		fftw_plan f_A = fftw_plan_dft_r2c_3d(dim_x, dim_y, dim_z, signal_A, fourier_A, 0);	// 0  ó  FFTW_ESTIMATE  ó  FFTW_MEASURE  http://www.fftw.org/doc/Complex-One_002dDimensional-DFTs.html
		...
		int i;
		for(block = 0;; block * DIMENSION_Z) { // Contabiliza la cantidad de energia de cada señal y su diferencia para cada porción de las señales, de tamaño block (cuadro del vídeo)
