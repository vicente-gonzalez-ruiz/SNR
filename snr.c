...
void compute_measures (	char **argv,
  ...
  if(FFT) {
    ...
    for(block = 0;; block++) {
      // Contabiliza la cantidad de energia de cada señal y su diferencia para cada porción de las señales, de tamaño block (cuadro del video)

      long long local_energy_error = energy_error, local_energy_A = energy_A;
      long long local_counter = counter;
      ...
      // Inserta la señal A en la estructura de datos fftw
      if(min==0) break; {
	for(i=0; i<min; i++) {
	  signal_A[i] = buf_A[i];
	}
	...
	spectrum_A[i] = sqrt(fourier_A[i][0]*fourier_A[i][0] + fourier_A[i][1]*fourier_A[i][1]);
      ...
      // Inserta la señal B en la estructura de datos fftw
      if(min==0) break; {
	for(i=0; i<min; i++) {
	  signal_B[i] = buf_B[i];
	}
